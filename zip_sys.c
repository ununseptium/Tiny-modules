...
void *zip_sys_get_pre_eocd_data(uint16_t *pre_extra_data_eocd_size, uintmax_t cdfh_offset, uintmax_t cdfh_total, uintmax_t cdfh_size){
	...
	*pre_extra_data_eocd_size =
		...
		sizeof(struct zip64_end_of_central_directory_locator) +
	...
	memcpy(
			pre_extra_data_eocd + sizeof(struct zip64_end_of_central_directory) + sizeof(struct zip64_end_of_central_directory_locator),
			zip64_data_sector,
			zip64_data_sector_size
	);
...
static void zip_sys_free_filedata_fields(filedata_t *fd){
	if (fd->relative_filename != NULL){
		free(fd->relative_filename);
		fd->relative_filename = NULL;
	}
	if (fd->absolute_filename != NULL){
		free(fd->absolute_filename);
		fd->absolute_filename = NULL;
	}
	if (fd->os_data != NULL){
		free(fd->os_data);
		fd->os_data = NULL;
...
uint32_t zip_sys_get_cdfh_count(FILEOS* archive, uintmax_t* count){
	if (archive == NULL || count == NULL) return 1;
	...
	if (eocd.totalCentralDirectoryRecord != UINT16_MAX){
		*count = eocd.totalCentralDirectoryRecord;
		return 0;
	}
