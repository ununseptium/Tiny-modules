...
void zip_bo_reverse_uint64_amd64(uint64_amd64_t *uint64){
	if (uint64 != NULL){
		uint64->low_half = zip_bo_reverse_uint32(uint64->low_half);
		uint64->high_half = zip_bo_reverse_uint32(uint64->high_half);
	}
}
...
void zip_bo_le_zip64_eocd(struct zip64_end_of_central_directory *zip64_eocd){
	if (!zip_bo_is_le() && zip64_eocd != NULL){
		...
		zip_bo_reverse_uint64_amd64(&(zip64_eocd->sizeOfZip64EndOfCentralDirectory));
		...
		zip_bo_reverse_uint64_amd64(&(zip64_eocd->numberCentralDirectoryRecord));
		zip_bo_reverse_uint64_amd64(&(zip64_eocd->totalCentralDirectoryRecord));
		zip_bo_reverse_uint64_amd64(&(zip64_eocd->sizeOfCentralDirectory));
		zip_bo_reverse_uint64_amd64(&(zip64_eocd->centralDirectoryOffset));
	}
}
...
void zip_bo_le_zip64_eocdl(struct Zip64EndOfCentralDirectoryLocator *zip64_eocdl){
	if (!zip_bo_is_le() && zip64_eocdl != NULL){
		...
		zip_bo_reverse_uint64_amd64(&(zip64_eocdl->zip64EndOfCentralDirectoryOffset));
